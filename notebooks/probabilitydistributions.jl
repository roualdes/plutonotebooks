...
# ╔═╡ 0a4357e6-710d-4da0-9e65-e3286003b325
@htl("""<a id="most"></a>""") # TODO there's got to be a better way to do this
...
md"The gas station keypad example above contrasts nicely against the next two examples.  The gas station keypad's distribution is a non-uniform distribution of probability, whereas the other two examples represent a uniform distribution.  These three examples will thus help us generalize our concept of probability."

# ╔═╡ 227003ce-5470-4f04-93a5-6cbe84a2358d
md"A probability distribution is any function that assigns a number, referred to as probability, to (most[^1]) subsets of a sample space and also satisfies the axioms of probability, provided below.  It is common to think of probability as only a number, but in fact it is a function that assigns probability to subsets of ``S``, and we also often refer to the number this functions returns as a probability.  After presenting the Axioms of Probability, we will generalize probability from finite sample spaces to countable and uncountable spaces.  Then we will discuss some more complicated examples of probability distributions, to help ensure that we come away with a general concept of probability that can be more easily applied to new situations. After highlighting the distinction between probability distributions and their density function representations, this notebook concludes with some properties of probability."
...
md"Statisticians have a bad habit of reusing the same name for different functions, namely the letter P which I write in bold as ``\mathbb{P}``. In fact, we'll see this same issue again in the notebooks on density functions and distribution functions, where different density functions are named ``f`` and different distribution functions are named ``F``."

# ╔═╡ 0774a8f8-e6d1-44dc-b186-d481b713ba00
md"There are an uncountable number of probability distributions (density, and distribution functions).  It is common in the world of statistics to reference each one with ``\mathbb{P}`` (``f``, or ``F``).  One hopes that the specific probability distribution (density or distribution function) under discussion is clear from the context.  One also imagines that navigating such distinctions is more difficult when first learning this material, so keep this in mind as you read on."
...
md"Notice the Bernoulli distribution with probability of ``1`` set to ``p``, namely ``\textrm{Bernoulli}(p)``, actually specifies an entire family of distributions, where ``p`` is the parameter.  In applied statistics, there are two ways to use this distribution as a model.
...
2. Assume the parameter ``p`` is unknown and estimate it from data.

    With dataset \{1, 0, 1\}, we might estimate ``\hat{p} = 2 / 3``.  Even if these data came from a fair coin, the estimate ``\hat{p} = 2/3`` is not unreasonable given the data."
...
md"When a probability distribution's density function exists, and in these notebooks we will always assume the density function does exist, probability is defined to be area under a probability distribution's  density function within the set of interest.  See the notebook on [TODO add link to] Density Functions for more details on density functions.  For now, we will assume that a density function of a probability distribution is the function that enables the assignment of probability to subsets of the sample space."

# ╔═╡ 61d40e2b-3767-4cf7-ab4a-90e6997585e7
@htl("""<a id="support"></a>""") # TODO there's got to be a better way to do this

# ╔═╡ bde58343-0124-402d-9fef-fa3ee702d660
md"In general, probability distributions dictate the assignment of probability to (most[^1]) subsets of the sample space, ``A \subseteq S`` by calculating area under their density function.  Let ``f`` be the density function for probability distribution ``\mathbb{P}``, and let ``A \subseteq S``.  If ``S`` is finite or countable, then

$$\mathbb{P}[{A}] = \sum 1_A(x) f(x),$$

and if ``S`` is uncountable, then

$$\mathbb{P}[A] = \int 1_A(x) f(x) \mathrm{d}x.$$
	
In general, the sum is taken over the natural numbers ``\mathbb{N}`` and the integral over the real numbers ``\mathbb{R}``, such that the support[^2] of the density function and the indicator function together further restrict the sum and intergral to the set of interest ``A \subseteq S``."
...
md"[^1]: Technically probability can only be assigned to measurable sets.  Unfortunately, I will probably not mention this technicality at all times when I should.  TODO see reference instead."

# ╔═╡ bfa410b8-649b-41cd-b469-146c7085b243
@htl("""Back from whence you came <a href="#most">⤶</a>""")

# ╔═╡ f7743f89-2e37-4624-9713-57099da68cef
md"[^2]: Recall that the support of a density function is defined to be the set that ensures the density function is positive, ``\{x | f(x) > 0\}``."

# ╔═╡ 203a123f-cdf2-4b97-b1f4-b5f34cf15d15
@htl("""<a href="#support">⤶</a>""")
